...
class Process::GenerateSvm
  include Libsvm

  SERILIZED_CLASSIFIER_FILE = File.join(Rails.root, 'lib/assets/serialized_classifiers/svm_classifier')
  def self.generate
    ...
    training_inputs = combined_training_data.map { |instance|
      Node.features(instance[0..-2].map(&:to_f))
    }
  ...
  def self.test()
    ...
    control_test_data.each do |input|
      input.map!(&:to_f)
      result = classifier.predict(Libsvm::Node.features(*input))
      control_data_correct += 1 if result == 0
    end
